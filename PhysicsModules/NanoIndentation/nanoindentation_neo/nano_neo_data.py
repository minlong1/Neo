"""
Nanoindentation data loading and preprocessing.

Supports Hysitron ('HYS') and iMicro ('IMICRO') export formats, falling back
to plain headerless CSV ('NAN'). Preprocessing extracts the unloading
segment (from peak displacement onward) and slices it between the
data_cutoff limits, which is the region the Oliver-Pharr model is fit to.

Ported from nano_indent (nano_neo_data.py).
"""
...
class NanoIndent_Data:
    ...
    def check_line(self, line_number, string, accepted_type):
        return line_number == string

    def check_file_type(self):
        if self.check_Hysitron_file():
            return
        if self.check_iMicro_file():
            return
    ...
    def read_data(self):
        if self._ftype == "HYS":
            self._rawdata = np.genfromtxt(
                self._file, delimiter="\t", skip_header=self.label + 1
            )
            ...
            with open(self._file, "r") as f:
                ...
                for i, line in enumerate(lines):
                    if i > 1:
                        raw_line = line.split(",")
                        self._rawdata[i - 2, :] = [
                            float(raw_line[1]) - std_dis,
                            float(raw_line[2]) - std_force,
                            float(raw_line[3]),
                        ]
        else:
    ...
    def pre_processing(self, limits=(0.1, 0.9), plot=False):
        ...
        # Unloading segment: from peak displacement to the end
