...
def read_feff_file(file: str, replace: bool = True) -> None:
    # Read the file
    try:
...
def calculate_nfeff(dest_path: Union[str, Path]) -> int:
    """
...
def find_feff8l_larch(file, copy2fold=False) -> Path:
    ...
    if copy2fold:
        ...
        src_file_path = Path.cwd().joinpath(folder_path)
        dest_file_path = Path.cwd().joinpath(
            folder_path.with_suffix("").joinpath("feff.inp")
        )
