...
- `Solvers/` — physics-agnostic optimization algorithms (numpy-only; **must never import larch or any physics module**). `core/` holds the framework (`ParameterSpace`/`GeneRange`/`ContinuousGeneRange`, `Individual`, `Population`, `OptimizationProblem`, `RunState`, `SolverResult`, `BaseSolver`, `refinement.py:local_search_refine` — genome-agnostic periodic polish of one or more individuals, `perturb_scale` is a *fraction of each gene's own range*, not a flat width); `ga/` the genetic algorithm operators and `GASolver`/`GARechenbergSolver`; `de/` Differential Evolution (DE/rand/1/bin) as both a `BaseSolver` (`DESolver`) and a standalone `differential_evolution_step(population, F, CR)` function physics modules can call per-generation directly; `pso/` global-best Particle Swarm Optimization (`PSOSolver`, Kennedy & Eberhart 1995 with Clerc & Kennedy 2002 constriction coefficients; optional `local_search` option periodically polishes the current best individual via `core/refinement.py`); `cmaes/` (mu/mu_w,lambda)-CMA-ES (`CMAESSolver`, Hansen & Ostermeier) — covariance matrix `C` adapts per-gene/per-correlation step scale, initialized from each gene's own `(high-low)/4` rather than identity, added specifically because `DE_MCMC`'s fixed MCMC noise scale performed badly on an EXAFS solver benchmark's heterogeneous per-gene ranges (e.g. `sigma2`'s entire valid range is far narrower than `s02`'s); `demcmc/` DE-BNN (see below). `Solvers/__init__.py` has the solver registry (`get_solver("GA")`, numeric IDs 0/1/2 preserved from historical `solOpt` values; `PSO`, `CMA_ES`, and `DE_MCMC` are string-keyed only, no numeric ID — none has a historical `solOpt` value).
...
**Back-compat shim layer**: `neoSelector.py`, `neoCrossOver.py`, `neoMutator.py`, `neoSolver.py` keep the historical EXAFS API (`NeoSelector().initialize(exafs_pars)` / `.select(pops)` etc.) but delegate the algorithms to `Solvers.ga`/`Solvers.de`. Preserved quirks the tests pin: `mutator.mutOpt == mut_options + 1`, exact `mutType`/`croType`/`solver_operator` label strings. Don't "fix" these without updating the tests. `neoSolver.py:NeoSolver_DE` (`solOpt == 2`) delegates to `Solvers.de.differential_evolution_step(pops, F, CR)` — `NeoPopulations` already exposes the `.problem`/`.generate_individual()`/`.eval_population()` surface that function needs, so no EXAFS-specific DE math lives there.

**GA_Rechenberg's mutation-chance adaptation is live**, not a no-op: `neoSolver.py:NeoSolver_GA_Rechenberg.rechenberg_mutation` writes the 1/5-success-rule update to `neo_pars.mutPars.mutChance` (gated to `currGen > 20` by `Solvers.ga.ga_solver.rechenberg_update`), and `neoMutator.py:NeoMutator.mutate()` re-syncs the active operator's `mut_chance`/`mutChance` from `mutPars.mutChance` on every call, so the adaptation actually reaches the mutator each generation. This used to be a real gap — the operator only read `mutChance` once, at `initialize()`, so the adapted value sat inert in `NeoPars` and GA_Rechenberg behaved identically to plain GA for an entire run (confirmed via a 10-seed benchmark on the EXAFS Cu fixture: bit-identical final loss across every seed). Fixed by the `mutate()` resync; see `PhysicsModules/EXAFS/tests/test_neomutator.py:test_neomutator_resyncs_mutchance_every_call` and `test_neosolver.py:test_ga_rechenberg_adaptation_reaches_mutator` for regression coverage. The generic `Solvers.ga.GARechenbergSolver` (no EXAFS shim involved) was never affected — it already wrote straight to `self.mutator.mut_chance`.
