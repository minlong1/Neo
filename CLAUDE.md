...
- `PhysicsModules/AstroNeo/` — Astro Neo: fits one hardcoded X-ray CCD spectral model (`TBabs(TBabs*powerlaw + lsmooth(vapec + vapec + zashift*vacx2))`, an absorbed powerlaw + two-temperature APEC plasma + ACX2 charge-exchange component) via PyXspec, scored by the Cash statistic. Ported from a personal research script; unlike EXAFS/NanoIndentation's ports, the original's own GA/selector/crossover/mutator framework was dropped rather than shimmed — most of its crossover/mutation operators referenced an `Individual` API (`get_e0`, `get_path`, `exafsPathPars`) that never existed on this module's actual `Individual`, i.e. were dead/broken, not historical behavior worth preserving. Genome = the model's 15 free parameters (`astro_neo/model.py:PARAM_NAMES`), a flat-float-genome module like EXAFS/NanoIndentation, plugged into `Solvers` via `astro_neo/problem.py:AstroNeoProblem`. **PyXspec is not pip/conda-installable** (ships with a HEASOFT source build) — `problem.py`/`model.py` import `xspec`/`acx2_xspec` lazily so the rest of the module (and its non-PyXspec tests) stay usable without it; see `PhysicsModules/AstroNeo/README.md` for manual setup and "Known limitations" (the model expression itself isn't a runtime choice, same as upstream).
